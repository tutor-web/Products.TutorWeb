...
def twresetlecture_questtionandslide(self):
    ...
    #for lec in lectureids:
            ...
            #l = lec.getObject()
    slideids  = container.getFolderContents(contentFilter={"portal_type": "Slide"})
	    
    questionids = container.getFolderContents(contentFilter={"portal_type": "TutorWebQuestion"})
    # set the slide
    for s in slideids:
        'bla bla'
        SlObj = s.getObject() 
    ...
    return 'finished transforming latex text on slides and questions ' + str(len(slideids)) + ', ' + str(len(questionids))

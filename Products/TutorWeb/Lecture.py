    ...
    def writeLectureGrades(self):
        ...
        if (len(studentdict) > 0):
            ...
            for stud in students:
                ...
                if (res):
                    if res.quizattempts > 0:
                        try:
                            email = studentdict[stud][1]
                        except:
                            email = ""
                        try:
                            randomid = studentdict[stud][0]
                        except:
                            randomid = ""
                        wholelist = wholelist + stud + '\t' + email + '\t' + randomid  + '\t' +  str(res.studentgrade) + '\t' + str(res.correctquizattempts) + '\t' + str(res.quizattempts) + '\t' + str(res.lasteight) + '\n' 
    ...
    def getQuizQuestion(self, tex_fd = 'tempfile', debug=False):
        ...
        if (numlectures > 0):
            ...
            if utmp <= hsp:
                ...
                num=random.randint(0.,lecindex+1)
#                while (num <= lecindex):
                ...
                testquestions = portal_catalog.unrestrictedSearchResults({'portal_type' : 'TutorWebQuestion'},path='/'.join(lectures[num].getPhysicalPath())) 
#                    for q in questions:
            ...
            for i in slides:
                ...
                if (len(explanationtext) > 0 or haveExpImage):
                    try:
                        ...
                        self.setSlideExpl(tex_fd, textfilename, explanation_text_type, explanationtext, explanation_image, i.getExplanationImageFormat(), tmpout, expimage_size, explcaption)
                        # REM ########
...
####    def setSlideExpl(self, tex_fd, textfilename, texttype, slidetext, tmpout):
        ...
        haveImage = (slideimage != 'FAILURE' and slideimage) 
        if (len(slidetext) > 0 and haveImage):
            leftwidth=0.58    # Widths when both text and figures
            rightwidth=0.38
        elif (len(slidetext) > 0 and (not haveImage)):
            rigthwidth=0
            leftwidth=0.97
...
###        os.close(t_fd)
    ...
    def setHeading(self, tex_fd):
        ...
        os.write(tex_fd, '\\usepackage{etex}\n')
        os.write(tex_fd, '\\usepackage{wrapfig}\n')
        os.write(tex_fd, '\\usepackage{pictexwd}\n')

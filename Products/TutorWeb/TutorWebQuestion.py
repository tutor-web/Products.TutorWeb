...
from Products.Archetypes.public import Schema, BooleanField, BooleanWidget, \
     ...
     TextAreaWidget, StringWidget, RichWidget, SelectionWidget, ImageWidget, ImageField
...
#from plone.app.textfield import *
...
class TutorWebQuestion(ATFolder):
    '''a tutorweb question '''
    archetype_name = portal_type = meta_type = 'TutorWebQuestion'
    ...
    schema = ATFolderSchema.copy() + Schema((
         StringField('id',
                  widget=StringWidget(description='By default ids are created automatically, using ids of the form q-001 upto q-999. Only change if want to override this setting. Questions appear in alphabetical order based on this value.', modes='edit',),
                  required = 1,
                  default='Q-000',
                  validators=('isSameQuestionId',), 
                  #default='Q-000',
                  ),   
         StringField('title',
                required=True,
           ...
           StringField('ImageUrl',
                default='',
                widget=StringWidget(label='Image location',
                                       description='Url or file location of an image to be shown along with the question text (http://www.myurl/myimage)',),
                validators=('isURL',), 
                ),
          ImageField('QuestionImage',
                   ...
                   widget=ImageWidget(label='Question image',
                                      description='Main image for question, displayed to the right of main text of the question. Possible formats for uploaded images are: png, gif and jpeg.', 
                                      #macro='tutorwebimage',
         ...
         # additional explanation to the question/answer
         TextField('questionExplanation', # See 'description' property
                                        # of the widget.
               searchable=0,
               allowable_content_types=('text/plain',
                   'text/latex',
                ...
                mutator='setQuestionExplanationText',
                accessor='questionExplanationRaw',
                widget=RichWidget(
                    label='A short explanation to the question',
                    macro='tutorwebquestion',
                    modes='edit',
                    visible={'view':'invisible'},
                    description='Provides additional information for students.',
                    rows=10,
                    ),   
            ),
       ##RichText(
       ...
       ##     allowed_mime_types=('text/plain', 'text/latex',),
       ##     default=defaultBody,
       ## ),
          TextField('quizQuestionExplanation', # See 'description' property
                ...
                widget=RichWidget(
                    label='A short explanation to the question.',
                    description='Provide additional information for students.',
                    visible={'edit':'invisible'},
                    ),
    ...
    def post_validate(self, REQUEST=None, errors=None):
        '''check if answers are set correctly when question is created/edited'''
        # start by finding out the format of the question text
        # since questions using r format generate their own answers
        # and therefore no error check needed
        field = self.getField('question')
        widget  = field.widget
        ...
        # have set the question field, can finally get the real format value
        texttype = field.getContentType(self)
        if ((texttype=='text/r') or (texttype=='text/r-latex')):
            # no answers required, continue
            return errors
        else:
            # get the value of the answers
            field = self.getField('AnswerList')
            widget = field.widget
            result = field.widget.process_form(self, field, form, empty_marker=_marker)
            if result is _marker or result is None:
                '''do nothing'''
            else:
            ...
            if (length < 1):
                ...
                errors['AnswerList'] = 'Please add at least one answer to the question'
            else:
                hassetcorrect = False
                rowcounter = 0
                for row in value:
                ...
                if (hassetcorrect==False):
            
                    errors['AnswerList'] = 'Please choose one answer as True'
                  
        return errors

    def getNumberAskedFor(self):
        return self.numaskedfor
    ...
    def setQuestionExplanationText(self, value, **kwargs):
        '''set short explanation text'''
        f = self.getField('questionExplanation')
        f.set(self, value, raw=True, **kwargs)
    def questionExplanationRaw(self):
        '''return viewable part- hmmm, a bit strange'''
        return self.getQuizQuestionExplanation()
    def hasChanged(self):
        return self.changed
    def getShortExample(self):
        '''transform short example if question has been changed'''
        if self.changed:
            # generate a new short examples
            self.changed = False
            transformed = self.transformSettingOrigin(self.questionExplanation.getContentType(), self.getRawQuestionExplanation(), 'shortexample')
            if (transformed.endswith('<BR><HR>\n\n')):
                transformed = transformed[:-10]
            self.setQuizQuestionExplanation(transformed)
            self.quizQuestionExplanation.setContentType(self, 'text/html')
            self.reindexObject()
    def generateNewId(self):
        """Suggest an id for this question numbered from Q-001 to Q-999"""
        parent = self.aq_inner.aq_parent
        return parent.generateNewIdForQuestion()
    ...
    def editedObject(self, objtype=None):
        ...
        self.changed = True
        self.getShortExample()
        self.changed = False
        questlocator = getUtility(IQuestionLocator)
        questioninfo = questlocator.question_by_uid(self.UID())
        if (questioninfo):
    ...
    def transformRQuestion(self):
        if (self.toTransformR()):
            ...
            text = self.getRawQuestion()
            qtype = self.question.getContentType()
...
if PLONE_VERSION == 3:
    registerATCTLogged(TutorWebQuestion)
else:
    atapi.registerType(TutorWebQuestion, PROJECTNAME)
